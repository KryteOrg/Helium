...
class Window(QMainWindow):
	...
	def initUI(self):
		...
		self.back.move(0,0)
		self.back.setStyleSheet("border-image: url('" + home + "/.kryte/helium/back.png') 0 0 0 0 stretch stretch; transform: scale(0.6);")
		self.forw = QPushButton("", self)
		self.forw.resize(30,30)
		self.forw.move(30,0)
		self.forw.setStyleSheet("border-image: url('" + home + "/.kryte/helium/forward.png') 0 0 0 0 stretch stretch;")
		self.key = QPushButton("", self)
		self.key.resize(30,30)
		self.key.move(60,0)
		self.key.setStyleSheet("border-image: url('" + home + "/.kryte/helium/key.png') 0 0 0 0 stretch stretch;")
		self.home = QPushButton("", self)
		self.home.resize(30,30)
		self.home.move(90,0)
		self.home.setStyleSheet("border-image: url('" + home + "/.kryte/helium/home.png') 0 0 0 0 stretch stretch;")
		self.refresh = QPushButton("", self)
		self.refresh.resize(30,30)
		self.refresh.move(120,0)
		self.refresh.setStyleSheet("border-image: url('" + home + "/.kryte/helium/refresh.png') 0 0 0 0 stretch stretch;")
		self.extension = QPushButton("", self)
		self.extension.resize(30,30)
		self.extension.move(150,0)
		self.extension.setStyleSheet("border-image: url('" + home + "/.kryte/helium/extension.png') 0 0 0 0 stretch stretch;")
		self.cookie = QPushButton("", self)
		self.cookie.resize(30,30)
		self.cookie.move(240,0)
		self.cookie.setStyleSheet("border-image: url('" + home + "/.kryte/helium/cookie.png') 0 0 0 0 stretch stretch;")
		self.search = QPushButton("", self)
		self.search.resize(30,30)
		self.search.move(270,0)
		self.search.setStyleSheet("border-image: url('" + home + "/.kryte/helium/search.png') 0 0 0 0 stretch stretch;")
		self.setGeometry(300,300,800,600)
		self.setWindowTitle('Helium Web Browser')
		self.show()
		self.tab.tabBar().tabButton(0, QTabBar.ButtonPosition.RightSide).resize(0,0)
		self.profile = QWebEngineProfile.defaultProfile()
		self.settings = self.profile.settings()
		self.settings.setAttribute(self.settings.WebAttribute.FullScreenSupportEnabled, True)
			...
			if entry.is_file():
				with open(entry.path) as f:
					try:
						...
						cookie.setValue(QByteArray(base64.b64decode(c["value"].encode("ascii")).decode("ascii")))
		...
		try:
			...
			self.createTab('file://' + home + "/.kryte/helium/start/index.html")
	...
	def handleSearch(self):
		...
		if (self.entry.text().startswith("https://") or self.entry.text().startswith("http://")):
			...
			self.tab.currentWidget().setUrl(QUrl('file://' + home + "/.kryte/helium/" + res))
	...
	def cookieAdded(self, cookie):
		...
		with open(home + "/.kryte/helium/cookies/" + cookie.name().data().decode("utf-8") + "-" + cookie.domain(), "w") as f:
			f.write('{"name": "' + cookie.name().data().decode("utf-8") + '","value": "' + base64.b64encode(cookie.value().data()).decode("ascii") + '","domain": "' + cookie.domain() + '","path": "' + cookie.path() + '"}')
	...
	def tabEvent(self):
		if (self.tab.currentIndex() == 0):
			self.createTab('file://' + home + "/.kryte/helium/start/index.html");
		elif (self.tab.currentWidget().url().toString().startswith("javascript:")):
			pass
		else:
			self.entry.setText(self.tab.currentWidget().url().toString())
			self.setWindowTitle(self.tab.currentWidget().page().title() + " - Helium Web Browser")
			self.tab.setTabText(self.tab.currentIndex(),self.tab.currentWidget().page().title())
			self.setWindowTitle(self.tab.currentWidget().page().title() + " - Helium Web Browser")
			if (self.tab.currentWidget().page().url().toString().startswith("file://" + home + "/.kryte/helium/")):
				_, _, res = self.tab.currentWidget().page().url().toString().partition("browser/")
				self.entry.setText("browser://" + res)
			else:
				self.entry.setText(self.tab.currentWidget().page().url().toString())
			try: self.back.clicked.disconnect()
			except Exception: pass
			try: self.forw.clicked.disconnect()
			except Exception: pass
			try: self.refresh.clicked.disconnect()
			except Exception: pass
			self.back.clicked.connect(self.tab.currentWidget().back)
			self.forw.clicked.connect(self.tab.currentWidget().forward)
			self.refresh.clicked.connect(self.tab.currentWidget().reload)
	...
	def extev(self):
		self.tab.currentWidget().setUrl(QUrl('file://' + home + "/.kryte/helium/webextensions/index.html"))

	def cookiejv(self):
		global cookie_jar

		total = sum([len(files) for r, d, files in os.walk(home + "/.kryte/helium/cookies/")])
	...
	def keymanager(self):
		self.tab.currentWidget().setUrl(QUrl('file://' + home + "/.kryte/helium/passwords/index.html"))

	def homeev(self):
		self.tab.currentWidget().setUrl(QUrl('file://' + home + "/.kryte/helium/start/index.html"))

	def loadStart(self):
		g = os.listdir(home + "/.kryte/helium/extensions")
		for ex in g:
			print(ex)
			if (self.tab.currentWidget().page().url().toString() == "file://" + home + "/.kryte/helium/webextensions/index.html"):
				self.tab.currentWidget().page().runJavaScript("document.getElementById('extlist').innerHTML = document.getElementById('extlist').innerHTML + '<a href=\"../extensions/" + ex + "/window.html\">" + ex + "</a><br>'")
	...
	def loaded(self):
		...
		if (re.search(r"/browser/extensions/.*/window.html#writeFile,.*$",self.tab.currentWidget().page().url().toString())):
			_, _, res = self.tab.currentWidget().page().url().toString().partition("#writeFile,")
			...
			with open(home + "/.kryte/helium/extensions/" + unquote(writeSplit[0]), "w") as f:
				f.write(base64.b64decode(unquote(writeSplit[1]).encode("ascii")).decode("ascii"))
				f.close()

		if (re.search(r"/browser/settings/index.html#newSearch,.*$",self.tab.currentWidget().page().url().toString())):
			_, _, res = self.tab.currentWidget().page().url().toString().partition("#newSearch,")

			search = base64.b64decode(unquote(res.split(",")[0]).encode("ascii")).decode("ascii")
